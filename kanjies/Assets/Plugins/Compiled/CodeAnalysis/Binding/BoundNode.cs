...
namespace Dar.CodeAnalysis.Binding
{
    internal abstract class BoundNode
    {
        ...
        private IEnumerable<PropertyPair> GetProperties()
        {
            var properties = GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance);
            foreach (var property in properties)
            {
                ...
                if (value != null)
                    yield return new PropertyPair { Name = property.Name, Value = value };
            }
        }
        ...
        {
            var b = node as BoundBinaryExpression;
            if (b != null)
                return b.Op.Kind.ToString() + "Expression";

            var u = node as BoundUnaryExpression;
            if (u != null)
